...
def train_sequences(model, dataloader, loss_fns, loss_weights, optimizer, num_epochs, status_interval, 
                    save_interval, model_metadata, log_file, device="cpu") -> None:
    """
    ...
    :param log_file: The log file to write the model status to
...
if __name__ == "__main__":        
    ...
    JSON_CORPUS = "data/corpus1.json"        # The JSON corpus to use
    LOG_FILE = "data/output.log"             # The output log
    ...
    # Train the model
    log.write(f"Training for {NUM_EPOCHS} epochs...\n")
    log.close()
    train_sequences(model, dataloader, loss_fn, loss_weights, optimizer, NUM_EPOCHS, PRINT_UPDATE_INTERVAL, MODEL_SAVE_INTERVAL, model_metadata, LOG_FILE, device)
